"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var router_deprecated_1 = require('@angular/router-deprecated');
var portfolio_service_1 = require('./portfolio.service');
var PortDetailComponent = (function () {
    function PortDetailComponent(portfolioService, routeParams) {
        this.portfolioService = portfolioService;
        this.routeParams = routeParams;
    }
    PortDetailComponent.prototype.ngOnInit = function () {
        var _this = this;
        var port = +this.routeParams.get('port');
        console.log('port' + port);
        this.portfolioService.getPortfolio(port)
            .then(function (portfolio) { return _this.portfolio = portfolio; });
    };
    PortDetailComponent.prototype.goBack = function () {
        window.history.back();
    };
    PortDetailComponent = __decorate([
        core_1.Component({
            selector: 'my-port-detail',
            templateUrl: 'app/template/hero-detail.component.html',
            styleUrls: ['app/css/hero-detail.component.css']
        }), 
        __metadata('design:paramtypes', [portfolio_service_1.PortfolioService, router_deprecated_1.RouteParams])
    ], PortDetailComponent);
    return PortDetailComponent;
}());
exports.PortDetailComponent = PortDetailComponent;
//# sourceMappingURL=port-detail.component.js.map